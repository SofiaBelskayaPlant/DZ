...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите пункт назначения:');
    const nomer = prompt('Введите груз:');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новый пункт назначения', oldTaskName[0]);
    const newNomer = prompt('Введите новый груз', oldTaskName[1]);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Рейс '${taskName}' будет удален. Продолжить?`)) return;
  ...
  render() {
    ...
    button.innerHTML = 'Добавить рейс';
